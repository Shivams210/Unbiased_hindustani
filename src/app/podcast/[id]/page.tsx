...
export default function PodcastDetailPage({ params }: { params: { id: string } }) {
  ...
  const handleVideoClick = () => {
    ...
    try {
      if (podcast.youtubeUrl.includes('/embed/')) {
        videoId = podcast.youtubeUrl.split('/embed/')[1].split('?')[0];
      } else if (podcast.youtubeUrl.includes('watch?v=')) {
        const urlObj = new URL(podcast.youtubeUrl);
        videoId = urlObj.searchParams.get('v') || '';
      } else if (podcast.youtubeUrl.includes('youtu.be/')) {
        videoId = podcast.youtubeUrl.split('youtu.be/')[1].split('?')[0];
      }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-white mt-4">Loading podcast...</p>
        </div>
      </div>
    );
  }
  ...
  if (!podcast) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-4xl font-bold text-white mb-4">Podcast Not Found</h1>
          <button
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-4xl mx-auto">
        ...
        <article className="bg-gradient-to-br from-gray-800/90 to-gray-900/90 backdrop-blur-sm rounded-2xl shadow-2xl overflow-hidden border border-gray-700/50">
          ...
          <div className="p-8">
            <div className="flex items-center justify-between mb-4">
              <span className="text-xs font-bold px-3 py-1.5 rounded-full bg-gradient-to-r from-purple-500/20 to-pink-600/20 text-purple-300 border border-purple-500/40">
                🎙️ PODCAST #{podcast.id}
              </span>
              {podcast.date && (
            ...
            <h1 className="text-4xl md:text-5xl font-bold mb-4 bg-gradient-to-r from-purple-400 via-white to-pink-500 bg-clip-text text-transparent leading-tight">
              {podcast.title}
            </h1>
            ...
            <div className="flex items-center mb-6">
              ...
              <span className="text-xl text-orange-300 font-semibold">{podcast.speaker}</span>
            </div>
            ...
            >
              <img
                src={getYouTubeThumbnail(podcast.youtubeUrl)}
                alt={podcast.title}
            ...
            <div className="mb-8">
              ...
              <div className="space-y-4">
                {podcast.summaryPoints.map((point, index) => (
                  <div key={index} className="flex items-start bg-gray-800/50 p-4 rounded-lg border border-gray-700/30">
                    <span className="flex-shrink-0 w-8 h-8 flex items-center justify-center bg-gradient-to-br from-purple-500 to-pink-600 text-white font-bold rounded-full mr-4">
                      {index + 1}
                    </span>
        ...
        <div className="mt-8 text-center">
          <button
            onClick={() => router.push('/podcasts')}
            className="px-8 py-4 bg-gradient-to-r from-purple-500 to-pink-600 text-white font-semibold rounded-lg hover:from-purple-600 hover:to-pink-700 transition-all duration-300 transform hover:scale-105 shadow-lg hover:shadow-xl"
          >
